...
|------|-------------|------------|
| `draft_campaign` | Create full campaign structure preview (budget + campaign + ad group + optional keywords) | `campaign_name`, `daily_budget` (budget cap enforced), `bidding_strategy`, keywords validated for match type safety |
...
3. **Pre-write checks (CRITICAL):**
   - Has the user specified a bidding strategy? If not, recommend MAXIMIZE_CONVERSIONS or TARGET_CPA over MANUAL_CPC. Never default to MANUAL_CPC.
...
3. Call `draft_campaign` with campaign name, daily budget, bidding strategy, ad group name, and optional keywords
4. Review the preview and any `warnings` (budget sufficiency, MANUAL_CPC warning, BROAD match rejection)
5. Present the complete preview to the user — emphasize the campaign will be created as PAUSED
6. After campaign creation, remind the user to:
